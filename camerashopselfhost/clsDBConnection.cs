...
namespace camerashopselfhost
{
    ...
    {
        private static ConnectionStringSettings ConnectionStringSettings = ConfigurationManager.ConnectionStrings["camerashop"];
        ...
        public static DataTable GetDataTable(string prSQL, Dictionary<string, Object> prPars)
        {
            ...
            using (DbCommand lcCommand = lcDataConnection.CreateCommand())
            {
                ...
                using (
                    DbDataReader lcDataReader = lcCommand.ExecuteReader(CommandBehavior.CloseConnection))
